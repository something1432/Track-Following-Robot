...
const unsigned int I_CONSTANT = 10;  //Inversly proportional constant
const unsigned int D_CONSTANT = 1000;  //Proportional constant
...
unsigned int I_time=0;  //Time counter for integral term
unsigned int D_time=0;  //Time counter for derivative term
...
void PID_LineFollowing(signed char error[])
{
    ...
    //Calculate I
    if (I_time == I_CONSTANT)  //Integral will only increment if Timer reads a multiple of I_CONSTANT with 4*128= 512 cycle tolerance
    {
